...
const PendaftarPerGelombang = () => {
  ...
  const { tipeGelombang } = useParams() as {
    tipeGelombang: "pembelian" | "pengembalian";
  };
  const queryClient = useQueryClient();
  const [searchName, setSearchName] = useState("");
  const [openedModal, { close: closeModal, open: openModal }] = useDisclosure();
  const [pendaftar, setPenndaftar] = useState<{
    id: number | null;
    name: string;
  } | null>(null);

  const { data: totalPendaftar, isLoading: loadTotalPendaftar } = useQuery({
  ...
  const { data: gelombang, isLoading: loadGelombang } = useQuery({
  ...
  const students: TColumnStudent[] = student?.data?.content?.map((item) => ({
    id: item?.id ?? 1,
    nama: item?.name ?? "-",
    noWa: item?.phone ?? "-",
    status: item?.status ?? null,
    tanggalMendaftar:
      tipeGelombang == "pembelian"
        ? item?.registration_Date ?? null
        : item?.registration_Date_Purchased_Batch ?? null,
        formulirId: item?.formulir_Id ?? "-",
  }));
  ...
  const columnHelper = createColumnHelper<TColumnStudent>();
  ...
  const columns = [
    ...
    // columnHelper.accessor((row) => row.formulirId, {
    //   id: "formulirId",
    //   cell: (info) => (
    //     <span style={{ whiteSpace: "nowrap" }}>{info.getValue()}</span>
    //   ),
    //   header: () => <span>Formulir ID</span>,
    // }),
    ...
    columnHelper.accessor((row) => row.tanggalMendaftar, {
      ...
      header: () => (
        <span style={{ whiteSpace: "nowrap" }}>Tanggal Mendaftar</span>
      ),
    }),
    columnHelper.accessor((row) => row.status, {
      ...
      cell: (info) => {
        ...
        if (valueStatus)
          return (
            <Badge size="lg" color={valueStatus.color} bg={!dark && "#dcfce2"}>
              {valueStatus.value}
            </Badge>
          );
        return "-";
      ...
      <Stack mt={20}>
            ...
            <Box>
              ...
              {loadTotalPendaftar ? (
                ...
                <Text
                  ...
                  align="center"
                >
                  {totalPendaftar?.data?.registered} Orang
                </Text>
              )}
            </Box>
            <Box>
              ...
              {loadTotalPendaftar ? (
                ...
                <Text
                  ...
                  align="center"
                >
                  {gelombang?.data?.max_quota} Orang
                </Text>
              )}
            </Box>
            <Box>
              ...
              {loadTotalPendaftar ? (
                ...
                <Text
                  ...
                  align="center"
                >
                  {totalPendaftar?.data?.diterima} Orang
                </Text>
              )}
            </Box>
          </Box>
        ...
        <Paper withBorder p="md" radius="md">
          <Group mb={20}>
            ...
            <Button onClick={() => sampleSubmitData(gelombangId)}>
              Export Excel
            </Button>
          </Group>
          {isErrorGetStudent ? (
            ...
            <>
              <DataTable
                ...
                usePagination={false}
                onSearch={(inputValue) =>
                  setSearchName(inputValue.toLowerCase())
                }
